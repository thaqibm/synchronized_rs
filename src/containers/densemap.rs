...
use std::{collections::hash_map::RandomState, marker::PhantomData, sync::{Mutex, RwLock}};
...
impl<K,V,S> LockDenseMap<K,V,S> {
    ...
    fn expand_needed(&self, probe_count: usize) -> bool {
        ...
        approx || 
        (probe_count > Self::PROBE_LIMIT && self.approx_inserts.get_accurate() > (self.data.len()/3) as u64)
    }
